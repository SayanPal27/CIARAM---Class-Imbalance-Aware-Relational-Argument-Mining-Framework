...
import shutil
import configparser
from utils import prepare_data, decode_anl
...
import argparse
from peft import get_peft_model, LoraConfig, TaskType, prepare_model_for_kbit_training, AutoPeftModelForSeq2SeqLM, PeftModel
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader, desc="Evaluating on Test Data", leave=False):
            ...
            for i in range(len(true_relations)):
                if(true_relations[i] != pred_relations[i]):
                    print("TRUE")
...
def main():
    for run in range(1, num_runs + 1):
        ...
        if do_train:
            ...
            intermediate_model = prepare_model_for_kbit_training(base_model)
            model = get_peft_model(intermediate_model, lora_config)
            ...
            current_step = 0
            output_dir = f"./results/qlora/{dataset_name}-{variant_name}-{model_name_or_path}-steps{num_steps}-SL{max_seq_length}-BS{batch_size}/run_{run}"
            ...
            if os.path.exists(best_checkpoint_path):
                ...
                if best_score_file:
                    with open(os.path.join(best_checkpoint_path, best_score_file[0]), 'r') as file:
                        ...
                        best_score = score_data["macro_f1"]
